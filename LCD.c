...
void LCD_Configure(void){
  ...
  // Wait until FCRSF flag of LCD_SR is set 
	// LCD->CR &= ~(LCD_SR_FCRSF);
	while ((LCD->SR & LCD_SR_FCRSF) != 0x00000020); // 020
  ...
  // Wait until the LCD is enabled by checking the ENS bit of LCD_SR 
	// LCD->SR &= ~(LCD_SR_ENS);
	while((LCD->SR & LCD_SR_ENS) != 0x00000001); // 001
	
  // Wait until the LCD booster is ready by checking the RDY bit of LCD_SR 
	// LCD->SR &= ~(LCD_SR_RDY);
	while((LCD->SR & LCD_SR_RDY) != 0x00000010); // 010
...
void LCD_MovingString(uint8_t* str, int len) {
	...
	// allocate memory for string with padded zeros
	//uint8_t *array;
	
	int j = 0;
	int k = 0;
	
	//*array = malloc((length + 6)* sizeof(uint8_t));
	
	// copy elements of string into allocated memory location
//	for(j; j < len; j++) {
//		
//		array[j] = *str;
//		str++;
//	}
	...
	// display 6 characters and shift
	for(k; k < len; k++) {
		
		int i = 0;
		
		LCD_DisplayString(str+k);
...
void LCD_Convert_Time(unsigned elapsed,uint8_t *converted) {
	...
	uint8_t seconds = (elapsed % 60000)/1000;
	uint8_t milliseconds = (elapsed % 60000 % 1000)/10 ;
	
	sprintf((char*)converted,"%.2u:%.2u.%.2u",minutes,seconds,milliseconds);
}
